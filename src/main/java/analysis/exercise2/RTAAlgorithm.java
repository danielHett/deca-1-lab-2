...
public class RTAAlgorithm extends CHAAlgorithm {
  ...
  private HashSet<ClassType> possibleTypes;
  ...
  protected void populateCallGraph(@Nonnull JavaView view, @Nonnull CallGraph cg) {
    this.possibleTypes = new HashSet<ClassType>();

    // First, need to collect information on possible classes in the program.
    for (JavaSootClass klaas : view.getClasses()) {
      for (JavaSootMethod m : klaas.getMethods()) {
        this.possibleTypes.addAll(getPossibleClassTypes(m));
      }
    }
  ...
  private ArrayList<MethodSignature> findPolymorphicMethodSignatures(MethodSignature targetMethodSig) {
    ...
    for (ClassType sub : subs) {
      ...
      // Don't add it if it isn't in possible classes.
      if (!this.possibleTypes.contains(sub))
        continue;
  ...
  private HashSet<ClassType> getPossibleClassTypes(JavaSootMethod targetMethod) {
    // If there's no body, then just return an empty list.
    if (!targetMethod.hasBody())
      return new HashSet<ClassType>();

    // We try to find all of the <init> call sites in the body.
    HashSet<ClassType> possibleTypes = new HashSet<ClassType>();
    for (Stmt stmt : targetMethod.getBody().getStmts()) {
      // First we try to get all method invokations. Three possibilities:
      // (1) we have an invoke statement.
      // (2) we have an assign statement with an invocation on the righthand side.
      // (3) none of the above.
      AbstractInvokeExpr invocationExpression = stmt instanceof JInvokeStmt ? ((JInvokeStmt) stmt).getInvokeExpr()
      ...
      // If it isn't an invocation, keep going.
      if (invocationExpression == null)
        continue;

      // We only care about init expressions
      if (!invocationExpression.getMethodSignature().getName().equals("<init>"))
        continue;

      // Only care about classes in the view.
      ClassType possiblePossibleType = invocationExpression.getMethodSignature().getDeclClassType();
      if (view.getClass(possiblePossibleType).isPresent())
        possibleTypes.add(possiblePossibleType);
    }
